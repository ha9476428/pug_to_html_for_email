...
/** Mọi trang .html nằm trực tiếp trong landing/ (không tính file trong css/js/images/design). */
...
async function main() {
  ...
  const pageJobs = pages
    ...
    .filter(({ pageName, jobs }) => {
      if (!jobs.length) {
        console.log(`— bỏ qua trang "${pageName}.html" (chưa có ảnh trong landing/design/${pageName}/)`);
        return false;
      }
      return true;
    });
  ...
  if (!pageJobs.length) {
    console.log(`\nChưa có ảnh thiết kế nào. Đặt file vào landing/design/<tên-trang>/<width>.png, vd:`);
    for (const p of pages) for (const w of BREAKPOINTS) console.log(`  landing/design/${p}/${w}.png`);
    process.exit(0);
  }
