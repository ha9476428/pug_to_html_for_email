...
chung trong `email/partials/` (`header.pug`, `footer.pug`) cũng là
...
1. Tạo `_figma-ten.pug`. Nếu demo 1 mixin (`email/mixins/`):

   ```pug
   +section({ padding: ['lg', 'lg', 'sm'] })
     +heading(2) Tên component
     +spacer('xs')
     +text({ color: theme.color.textMuted }) Mô tả ngắn — email/mixins/_xxx.pug

   +section({ padding: [0, 'lg', 'lg'] })
     +mixinName(...)
   ```

   Nếu demo 1 partial (`email/partials/`) — partial thường đã tự bọc
   ...
   +section({ padding: ['lg', 'lg', 'sm'] })
     +heading(2) Tên component
     +spacer('xs')
     +text({ color: theme.color.textMuted }) Mô tả ngắn — email/partials/ten.pug
