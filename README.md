...
└── figma/                    # component catalog — xem mục riêng bên dưới
    ├── _README.md
    ├── _design-buttons.pug ... _design-typography.pug
...
- Mỗi component 1 file `_design-ten.pug` (chỉ nội dung, không tự build riêng).
- `index.pug` **tự sinh** mỗi lần build: quét mọi `_design-*.pug` trong thư mục, include hết theo thứ tự alphabet của tên file. Thêm component mới = chỉ cần tạo file `_design-ten.pug`, không cần sửa `index.pug`.
